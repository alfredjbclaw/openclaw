...
- `gateway.auth.allowTailscale`: when `true`, Tailscale Serve identity headers can satisfy Control UI/WebSocket auth (verified via `tailscale whois`), plus the read-only user profile avatar route and the metadata-only Control UI bootstrap config read (`/control-ui-config.json`). Ambient identity on those reads requires same-origin browser evidence (`Origin` passing the Control UI origin policy, or `Sec-Fetch-Site: same-origin` when `Origin` is absent), and it grants no operator authority beyond read access — no declared-scope trust, no plugin frames, no plugin auth cookie. It also mints no capability: assistant-media metadata, media-ticket minting, and media bytes all require a real credential (token, trusted-proxy identity, paired device token) or the device-bound HTTP credential the gateway issues once the Control UI's WebSocket connect authenticates — see [Tailscale](/gateway/tailscale). Other HTTP API endpoints do **not** use that Tailscale header auth; they follow the gateway's normal HTTP auth mode instead. This tokenless flow assumes the gateway host is trusted. Defaults to `true` when `tailscale.mode = "serve"`.
