...
- `gateway.auth.allowTailscale`: when `true`, Tailscale Serve identity headers can satisfy Control UI/WebSocket auth (verified via `tailscale whois`), plus the read-only user profile avatar route, the metadata-only Control UI bootstrap config read (`/control-ui-config.json`), and the assistant-media metadata read (`meta=1`). Ambient identity on those reads requires same-origin browser evidence (`Origin` passing the Control UI origin policy, or `Sec-Fetch-Site: same-origin` when `Origin` is absent), and it grants no operator authority beyond read access — no declared-scope trust, no plugin frames, no plugin auth cookie. The assistant-media metadata read serves no file bytes and mints the short-lived, source-scoped media ticket the dashboard needs for byte fetches. The Control UI routes that serve local file bytes — assistant-media byte reads and Control UI avatars — stay excluded: they require the minted media ticket, a token, trusted-proxy identity, or paired device token. Other HTTP API endpoints do **not** use that Tailscale header auth; they follow the gateway's normal HTTP auth mode instead. This tokenless flow assumes the gateway host is trusted. Defaults to `true` when `tailscale.mode = "serve"`.
